...
# Default callback aligns with API prefix /api/v1/auth
OAUTH_CALLBACK_URL = os.getenv("OAUTH_CALLBACK_URL", "http://localhost:8000/api/v1/auth/google/callback")
...
async def get_current_user(credentials: HTTPAuthorizationCredentials = Depends(security)) -> dict:
    ...
    users = UserRepository(get_mongo())
    user = await users.find_by_email(payload.get("email"))
    if not user:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
...
async def google_callback(request: Request) -> RedirectResponse:
    ...
    try:
        ...
        # Find or create user
        users_repo = UserRepository(get_mongo())
...
async def update_user_profile(user_id: str, updates: dict) -> dict:
    ...
    users_repo = UserRepository(get_mongo())
    await users_repo.update_profile(user_id, updates)
