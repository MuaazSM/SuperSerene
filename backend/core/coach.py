...
def _sanitize_coach_output(text: str, max_words: int = 150, max_sentences: int = 5) -> str:
    """Clean LLM output, keep up to 5 sentences, and ensure a reflective question is present."""
    t = (text or "").strip()
    if not t:
...
def coach_question(state: Dict[str, Any], llm=None) -> str:
    ...
    if chat is not None:
        try:
            ...
            q = _sanitize_coach_output(raw, max_words=150, max_sentences=4)
            return q
        except Exception as e:
...
class CoachAgent:
    """Wrapper class for coaching functionality."""
    
    def __init__(self, llm=None):
        """Initialize CoachAgent with optional LLM."""
        self.llm = llm or _ensure_llm()
    ...
    async def generate_response(
        ...
        Returns:
            Dict with "response" key containing the coach's message
        """
        # For async compatibility, run the synchronous function in an executor
        import asyncio
        loop = asyncio.get_event_loop()
        
        # Create a simple response if input is empty
        if not user_input or not user_input.strip():
            return {
                "response": "I'm here to listen. What's on your mind today?"
            }
        
        # Use coach_followup to generate contextual response
        try:
            result = await loop.run_in_executor(
                None,
                lambda: coach_followup(
                    user_id=user_id,
                    last_exchange={
            ...
            response_text = result.get("coach_response", result.get("response", "I'm here to support you."))
            return {
            ...
            # Fallback response
            return {
                "response": f"I hear you. That's important. Tell me more about how you're feeling."
            }
    
    def coach_question(self, state: Dict[str, Any]) -> str:
        """
        Generate a coaching question based on state.
        
        Args:
            state: State dict with emotions, topics, etc.
            
        Returns:
            Coaching question string
        """
        return coach_question(state, self.llm)
    
    def rewrite_message(self, text: str, intent: str = "assertive_kind") -> Dict[str, Any]:
        """
        Rewrite a message with specific intent.
        
        Args:
            text: Text to rewrite
            intent: Intent type (assertive_kind, validating, etc.)
            
        Returns:
            Rewritten message dict
        """
        return rewrite_message(text, intent, self.llm)
